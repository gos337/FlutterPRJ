...
enum DesignType { Banner, FavTheme, TrainerTheme, BannerTheme, PartTrainingTheme, DT008, DT011 }

Map<DesignType, String> designTypeValues = {
  DesignType.Banner: "DT002",
  DesignType.BannerTheme: "DT004",
  DesignType.FavTheme: "DT006",
  DesignType.TrainerTheme: "DT007",
  DesignType.DT008: "DT008",
  DesignType.DT011: "DT011",
};

final designTypeEnumValues = EnumValues({
  "DT002": DesignType.Banner,
  "DT004": DesignType.BannerTheme,
  "DT006": DesignType.FavTheme,
  "DT007": DesignType.TrainerTheme,
  "DT008": DesignType.DT008,
  "DT011": DesignType.DT011,
});
...
class MainRes {
  ...
  Map GetFavThemeItem(int index) {
    ...
    // int themeIndex = 2;
    int? themeIndex = GetIndexOfDesigntype(designType: DesignType.FavTheme);
    ...
    if (result.mainLayouts[themeIndex].items[index].type == ItemType.C) {
      items = {
        "imageURL": result.mainLayouts[themeIndex].items[index].content?.thumbUrl,
  ...
  Map GetDesignType_008ThemeItem(int themeIndex, int index) {
    // List<Map<String, String>> data = List.empty(growable: true);
    Map items = <String, String?>{};
    // int themeIndex = 2;
    // int? themeIndex = GetIndexOfDesigntype(designType: DesignType.FavTheme);
    ...
    if (result.mainLayouts[themeIndex].items[index].type == ItemType.C) {
      items = {
        "imageURL": result.mainLayouts[themeIndex].items[index].content?.thumbUrl,
        "title": result.mainLayouts[themeIndex].items[index].content?.title,
        "key": result.mainLayouts[themeIndex].items[index].content?.key,
      };
    }
  ...
  Map GetTrainerThemeItem(int themeIndex, int index) {
    Map items = <String, String?>{};
    // int? themeIndex = GetIndexOfDesigntype(designType: DesignType.TrainerTheme);
    ...
    if (result.mainLayouts[themeIndex].items[index].type == ItemType.C) {
      items = {
        "key": result.mainLayouts[themeIndex].items[index].content?.key.toString(),
        "imageURL": result.mainLayouts[themeIndex].items[index].content?.thumbUrl,
        "title": result.mainLayouts[themeIndex].items[index].content?.title,
      };
    }
  ...
  Map GetBannerThemeItem(int index) {
    // List<Map<String, String>> data = List.empty(growable: true);
    Map items = <String, String?>{};
    int? themeIndex = GetIndexOfDesigntype(designType: DesignType.BannerTheme);
  ...
  Map GetPartTrainingThemeItem(int index) {
    // List<Map<String, String>> data = List.empty(growable: true);
    Map items = <String, String?>{};
    int? themeIndex = GetIndexOfDesigntype(designType: DesignType.PartTrainingTheme);
  ...
  int GetMainBannerItemCount() {
    int? i = GetIndexOfDesigntype(designType: DesignType.Banner);

    if (i != null) {
      return result.mainLayouts[i].items.length;
    }

    return 0;
  }

  int? GetIndexOfDesigntype({required DesignType designType}) {
    for (int i = 0; i < result.mainLayouts.length; i++) {
      if (result.mainLayouts[i].designType == designTypeValues[designType]) {
        return i;
      }
    }
  ...
  Map GetMainBannerItem(int index) {
    ...
    Map items = <String, String?>{};
    // int themeIndex = 1;
    int? themeIndex = GetIndexOfDesigntype(designType: DesignType.Banner);
    ...
    if (result.mainLayouts[themeIndex].items[index].type == ItemType.B) {
      items = {
        "imageURL": result.mainLayouts[themeIndex].items[index].boardItem?.imageUrl,
  ...
  int getThemeModuleCount() {
    return result.mainLayouts.length;
  }

  DesignType? getThemeModulType({required int index}) {
    String strDesignType = result.mainLayouts[index].designType;
    DesignType? designType = designTypeEnumValues.map[strDesignType];

    print("getThemeModulType : $strDesignType");
    return designType;
  }
}
...
class Result {
  ...
  factory Result.fromJson(Map<String, dynamic> json) => Result(
        ...
        mainLayouts: List<MainLayout>.from(json["main_layouts"].map((x) => MainLayout.fromJson(x))),
...
class MainLayout {
  ...
  Map GetMainBannerItem(int index) {
    Map resItems = <String, String?>{};

    if (items[index].type == ItemType.B) {
      resItems = {
        "imageURL": items[index].boardItem?.imageUrl,
        "title": items[index].boardItem?.title,
        "desc": items[index].boardItem?.desc,
      };
      // print(items);
    } else if (items[index].type == ItemType.C) {
      resItems = {
        "imageURL": items[index].content?.thumbUrl,
        "title": items[index].content?.title,
        "desc": items[index].content?.desc,
      };
    }
...
class Item {
  ...
  factory Item.fromJson(Map<String, dynamic> json) => Item(
        ...
        boardItem: json["board_item"] == null ? null : Notice.fromJson(json["board_item"]),
...
class Notice {
  ...
  factory Notice.fromJson(Map<String, dynamic> json) => Notice(
        ...
        title: json["title"],
        desc: json["desc"],
        eventRule: json["event_rule"] == null ? null : EventRule.fromJson(json["event_rule"]),
        startDt: json["start_dt"],
        endDt: json["end_dt"],
        imageUrl: json["image_url"],
        link: json["link"] == null ? null : Link.fromJson(json["link"]),
        applyItem: json["apply_item"] == null ? null : ApplyItem.fromJson(json["apply_item"]),
...
class Content {
  ...
  factory Content.fromJson(Map<String, dynamic> json) => Content(
        ...
        favoriteYn: json["favorite_yn"] == null ? null : ynValues.map[json["favorite_yn"]]!,
        level: json["level"] == null ? null : levelValues.map[json["level"]],
        previewSection: json["preview_section"] == null ? null : PreviewSection.fromJson(json["preview_section"]),
...
final levelValues = EnumValues({"L2": Level.L2, "L3": Level.L3, "L4": Level.L4, "L5": Level.L5});
...
final metaCodeValues = EnumValues({"ACTION": MetaCode.ACTION, "DETAIL_BODYPART": MetaCode.DETAIL_BODYPART});

enum MetaName { EMPTY, META_NAME }

final metaNameValues = EnumValues({"집중부위": MetaName.EMPTY, "포함동작": MetaName.META_NAME});
...
enum Code { ARM, CALF, THIGH, HIP, SHOULDER, CORE, LEGRAISE, CRUNCH, SQUAT, DEADLIFT, SUPERMAN, PUSHUP }
...
enum Name { EMPTY, NAME, PURPLE, FLUFFY, TENTACLED, STICKY, INDIGO, INDECENT, HILARIOUS, AMBITIOUS, CUNNING, PUSHUP }
...
final previewSectionTypeValues =
    EnumValues({"M": PreviewSectionType.M, "S": PreviewSectionType.S, "T": PreviewSectionType.T});
